...
//,  aubioOnsetDetectionVector.size()-1
	...
	//drawRussianDragonError(aubioWindow);
}


void LiveAudioAnalysis::drawRussianDragonError(WindowRegion window){

	if (recentRussianDragonError > 0)
	ofSetColor(255,0,0);
	else 
		ofSetColor(0, 255,0);
	
	double proportion = recentRussianDragonError / marginOfError;
	proportion *= window.width * 0.5;
	if (proportion > 0)
		ofRect(window.x + window.width*0.5 - proportion, 20, abs(proportion), 20);
	else if (proportion < 0)
		ofRect(window.x + window.width*0.5 , 20, abs(proportion), 20);
//	else
//		ofLine(window.x + window.width*0.5, 20, window.x + window.width*0.5, 20);
	
	ofSetColor(0,0,0);
	ofLine(window.x + window.width*0.5, 0, window.x + window.width*0.5, 60);
...
void LiveAudioAnalysis::drawBeatStripes(int minIndex){
	...
	for (int j = 0;beatIndex + j < beatFrames.size() && beatFrames[beatIndex + j] < startingFrame + scrollWidth;j++){
		...
		int screenPosition = (beatStripePosition - startingFrame)*screenWidth/scrollWidth;
		ofLine(screenPosition, 0, screenPosition, screenHeight);
			if (beatIndex + j - 1 >= 0)
			ofDrawBitmapString(ofToString(beatFrames[beatIndex+j] - beatFrames[beatIndex+j-1]), screenPosition+ 5, 60);
	
			if (beatIndex + j > 3){
				double bpm = framesToSeconds((beatFrames[beatIndex + j] - beatFrames[beatIndex+j-4])/4.);//
				bpm = secondsToBPM(bpm);
				ofDrawBitmapString(ofToString(bpm, 2), screenPosition+ 5, 90);
			}
...
//	maxIndex = min ((int)energyVec.size() - 1, maxIndex);
	...
	for (i = 0; i < scrollWidth - 1; i++){
		...
		if (j < energyVec.size()){
			xPosition = i*screenWidth/scrollWidth;
			newHeight =	screenHeight - (energyVec[j]*screenHeight/heightFactor);
			ofLine(xPosition, screenHeight - (energyVec[j]*screenHeight/heightFactor),
				   screenWidth*(i+1)/scrollWidth, screenHeight - (energyVec[j]*screenHeight/heightFactor));
			ofLine(xPosition,lastHeight, xPosition, newHeight);
...
//	aubioBufferSize
	if (onsetDetector->processframe(audioFrame, framesize)){
		...
		if (dfvalue > aubioMedianValue * 1.05 && 
			...
			double error = checkRussianDragon(aubioOnsetDetectionVector.size()-1);
			russianDragonError.push_back(error);
			if (error != -1000.0)
				recentRussianDragonError = error;
		}
...
double LiveAudioAnalysis::checkRussianDragon(int aubioFrame){
	marginOfError = beatPeriodEstimate/7;
	if (abs(predictedNextBeat - aubioFrame) < marginOfError)
		return (aubioFrame - predictedNextBeat);
	else if (abs(aubioFrame - recentBeat) < marginOfError)
		return (aubioFrame - recentBeat);
	else
		return -1000.0;
